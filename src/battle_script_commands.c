...
static void Cmd_various(void)
{
    ...
    case VARIOUS_PALACE_FLAVOR_TEXT:
            ...
            && !(gBattleMons[gActiveBattler].status1 & STATUS1_SLEEP))
        {
            gBattleStruct->palaceFlags |= gBitTable[gActiveBattler];
            gBattleCommunication[0] = TRUE;
            gBattleCommunication[MULTISTRING_CHOOSER] = sBattlePalaceNatureToFlavorTextId[GetNatureFromSpeciesAndPersonality(gBattleMons[gActiveBattler].species, gBattleMons[gActiveBattler].personality)];
        }
        break;
...
static void Cmd_hiddenpowercalc(void)
{
    ...
    // Subtract 4 instead of 1 below because 3 types are excluded
    // (TYPE_NORMAL, TYPE_MYSTERY, and TYPE_FAIRY)
